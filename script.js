...
let targetTextareaForUpload = null; // Guarda qual textarea receberá o texto do ficheiro


// --- FUNÇÕES DO BANCO DE DADOS (REPLIT DB) ---
// (Nenhuma alteração nesta secção)
async function saveChat(chatId, history) {
    try {
        await fetch('/db', {
            method: 'POST',
...
async function loadAndDisplayChatList() {
    try {
        const response = await fetch('/db?prefix=chat_');
        ...
        for (const key of keys.sort().reverse()) {
            const chatResponse = await fetch(`/db/${key}`);
            const chatData = await chatResponse.json();
            const title = chatData[0]?.parts[0]?.text.substring(0, 50) + '...' || 'Conversa sem título';
            ...
            listItem.addEventListener('click', () => loadChat(key));
...
async function loadChat(chatId) {
    try {
        const response = await fetch(`/db/${chatId}`);
        chatHistory = await response.json();
        currentChatId = chatId;
        if (isAgentMode) toggleView();
        renderChat();
        const title = chatHistory[0]?.parts[0]?.text || 'Conversa';
...
/** Lida com a seleção de um ficheiro, extrai o texto e o coloca na textarea alvo. */
    ...
    try {
        ...
        switch (extension) {
            ...
            case 'docx':
                ...
                const docxResult = await mammoth.extractRawText({ arrayBuffer: arrayBufferDocx });
            ...
            case 'pdf':
                ...
                break;
            default:
                alert('Tipo de ficheiro não suportado. Por favor, use .txt, .docx, ou .pdf.');
        }
        ...
        fileInput.value = ''; // Limpa o input para permitir o mesmo ficheiro novamente
    }
}
...
uploadContextBtn.addEventListener('click', () => {
    targetTextareaForUpload = initialContext;
    fileInput.click();
});

uploadChatBtn.addEventListener('click', () => {
    targetTextareaForUpload = chatInput;
    fileInput.click();
});


// --- RESTANTE DO CÓDIGO (UI, AGENTE, CHAT) ---
// (Nenhuma alteração significativa nesta secção, apenas pequenas adaptações)
...
async function runAgent() {
    ...
    for (let i = 0; i < steps.length; i++) {
        ...
        const responseText = await fetchFromGemini(chatHistory);
        showTypingIndicator(false);
        chatHistory.push({ role: "model", parts: [{ text: responseText }] });
        addMessageToUI('model', responseText, 'step-result');
    }
    addMessageToUI('system', '✅ Agente concluiu todas as etapas.', 'step-header');
    await saveChat(currentChatId, chatHistory);
...
chatForm.addEventListener('submit', async (e) => {
    ...
    showTypingIndicator(true);
    const aiText = await fetchFromGemini(chatHistory);
    showTypingIndicator(false);
    ...
    if (chatHistory.length === 2) {
        await loadAndDisplayChatList();
        document.querySelector(`li[data-chat-id="${currentChatId}"]`)?.classList.add('active-chat');
    }
});
